...
### Fixed
- **OAuth2 service-account responses missing `displayName`**: OAuth2 client-credentials authenticates as a Keycloak service account — a JIT Alfresco user with no display name — so Alfresco returns `createdByUser` / `modifiedByUser` / `owner` without the (spec-required) `displayName`, and the generated `UserInfo.from_dict` raised `KeyError('displayName')`. A runtime shim (`python_alfresco_api/_compat.py`, applied at package import) now defaults `displayName` to `id` when it's absent, for the core and search clients' `UserInfo`. It lives outside `raw_clients/` so it survives client re-generation. (Basic/ticket and user-based OAuth2 tokens were unaffected — those users have a display name.)
