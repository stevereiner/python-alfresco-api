"""
Level 3: Ratings Operation Models - Specific to Ratings Operations

This module contains models that are specific to ratings operations
within the Core API.

Three-Tier Architecture:
- Level 1: Global models shared across ALL APIs  
- Level 2: Core API models shared within Core API
- Level 3 (This file): Ratings operation-specific models

Benefits:
- Perfect locality (ratings models exactly where ratings operations are)
- Clean imports (from .models import RatingsResponse)
- Logical organization (operation-specific grouping)
"""

from typing import Optional, List, Dict, Any, Annotated
from datetime import datetime
from pydantic import BaseModel, Field, ConfigDict

# Import from Level 1 (global)
from ...models import BaseEntry, ContentInfo, UserInfo

# Import from Level 2 (Core API)
from ..models import CoreResponse


class RatingsResponse(CoreResponse):
    """Response wrapper for ratings operations."""
    entry: BaseEntry = Field(..., description="Ratings data")


class RatingsListResponse(BaseModel):
    """Response wrapper for ratings list operations."""
    model_config = ConfigDict(extra='forbid')
    
    list: Dict[str, Any] = Field(..., description="List data with pagination")


# Operation-specific models will be added here based on analysis
# Example models for common patterns:


class CreateRatingsRequest(BaseModel):
    """Request model for creating ratings."""
    model_config = ConfigDict(extra='forbid')
    
    name: Annotated[str, Field(
        description="Ratings name",
        min_length=1,
        max_length=255
    )]
    
    properties: Annotated[Optional[Dict[str, Any]], Field(
        description="Custom properties",
        default=None
    )]


class UpdateRatingsRequest(BaseModel):
    """Request model for updating ratings."""
    model_config = ConfigDict(extra='forbid')
    
    name: Annotated[Optional[str], Field(
        description="Updated name",
        min_length=1,
        max_length=255,
        default=None
    )]
    
    properties: Annotated[Optional[Dict[str, Any]], Field(
        description="Updated properties",
        default=None
    )]


# Export all models
__all__ = [
    'RatingsResponse', 
    'RatingsListResponse',
    'CreateRatingsRequest',
    'UpdateRatingsRequest'
]